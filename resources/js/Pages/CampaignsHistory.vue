  ...
  <app-layout>
    <template #header>
      <h2 class="font-semibold text-xl text-gray-800 leading-tight">
        История ежедневной проверки по кампаниям
      </h2>
    </template>
    <div class="py-12">
      <div class="max-w-9xl mx-auto sm:px-6 lg:px-8">

        <div class="p-6 sm:px-20 bg-white border-b border-gray-200 mb-2">
          <div class="mt-2 text-2xl">
            История еженедельной проверки робота, результаты сгруппированы по кампаниям.
          </div>
          <div class="mt-6 text-gray-500">
            На данной странице отображены результаты проверок всех кампаний следующим образом : за один день
            сгруппированы все записи каждой кампании. <br>
        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg pt-4 pb-4">
          <div v-if="isExpanded"
               class="align-middle inline-block min-w-full overflow-hidden bg-white shadow-dashboard px-8 pt-3">
                   <span @click="isExpanded = false"
                         class=" relative inline-block px-3 py-1 font-semibold text-green-900 leading-tight cursor-pointer float-right mb-2">
                      <span aria-hidden class="bg-green-200 absolute inset-0 rounded-full"></span>
                      <span class="relative text-xs">Назад</span>
                   </span>
            <table class="min-w-full mb-80 table-fixed">
              <thead>
              <tr>
              ...
              <tr class="result-table-item" v-for="campaignLog of campaignLogs" v-bind:key="campaignLog.id">
                ...
                <td class="px-3 py-4 text-center border-b text-blue-900 border-gray-500 text-sm leading-5 ">
                                    <span
                                        class=" relative inline-block px-3 py-1 font-semibold text-green-900 leading-tight cursor-pointer">
                                    <span
                                        v-bind:class="[campaignLog.type == 'processed' ? 'bg-green-200' :  campaignLog.type == 'error' ? 'bg-red-600' : 'bg-yellow-200', '']"
                                        aria-hidden class="absolute inset-0 rounded-full"></span>
                                    <span v-if="campaignLog.type == 'processed'"
                                          class="relative text-xs">processed</span>
                                    <span v-else-if="campaignLog.type == 'error'" class="relative text-xs text-white">error</span>
                                    <span v-else-if="campaignLog.type == 'warning'"
                                          class="relative text-xs">warning</span>
          ...
          <div v-else class="align-middle inline-block min-w-full overflow-hidden bg-white shadow-dashboard px-8 pt-3">
            ...
            <table class="min-w-full">
              ...
              <tr>
                <th @click="changeSort('Date')"
                  ...
                  <div class="inline-flex">
                    <div class="inline-block">Дата</div>
                ...
                <th @click="changeSort('task_id')"
                    class="cursor-pointer px-3 py-3 border-b-2 border-gray-300 text-left text-sm leading-4 text-blue-500 tracking-wider">
                  <div class="inline-flex">
                    <div class="inline-block">Id проверки</div>
              ...
              <tr class="result-table-item" v-for="campaign of campaignList" v-bind:key="campaign.id">
                ...
                <td class="px-3 py-2  border-b text-blue-900 border-gray-500 text-sm leading-5 text-left">
                                  <span
                                      class=" relative inline-block px-3 py-1 font-semibold text-green-900 leading-tight cursor-pointer">
                                     <span aria-hidden v-bind:class="[campaign.warnings > 0 ? 'bg-yellow-200' : '']"
                ...
                <td class="px-3 py-2  border-b text-blue-900 border-gray-500 text-sm leading-5 text-left">
                                  <span
                                      class=" relative inline-block px-3 py-1 font-semibold text-green-900 leading-tight cursor-pointer">
                                     <span aria-hidden v-bind:class="[campaign.errors > 0 ? 'bg-red-200' : '']"
                                           class="absolute inset-0 rounded-full"></span>
                ...
                <td class="px-3 py-2  border-b text-blue-900 border-gray-500 text-sm leading-5 text-right">
                                  <span @click="getCampaign(campaign.campaign_id, campaign.date)"
                                        class=" relative inline-block px-3 py-1 font-semibold text-green-900 leading-tight cursor-pointer">
                                  <span aria-hidden class="bg-green-200 absolute inset-0 rounded-full"></span>
            ...
            <div v-if="isLoad" class="text-center">
              <img class="relative inline-block" src="/img/loading_2.gif">
            </div>
...
export default {
  ...
  methods: {
    ...
    getHistory(page = 1) {
      ...
      axios({
        method: 'post',
        url: '/api/v1/history/campaigns/list',
        data: {
        ...
        // Set campaign list
        app.campaignList = resp.data.data;
    ...
    getCampaign(campaign_id, date) {
      console.log(campaign_id, date);
      var app = this;
      app.campaignLogs = [];
      this.isExpanded = !this.isExpanded;
      axios({
        method: 'get',
        url: '/api/v1/history/campaign/' + campaign_id + '/' + date,
        data: {}
      }).then(function (resp) {
        app.campaignLogs = resp.data;
      }).catch(function (resp) {
        console.log(resp);
        alert("Could not load logs !");
      }).then(function () {
